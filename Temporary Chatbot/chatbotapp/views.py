...
def image_and_prompt_to_text(request):
    """
    View that handles the image upload and prompt submission to Gemini API.
    """
    if request.method == "POST":
        form = ImageUploadForm(request.POST, request.FILES)
        prompt = request.POST.get('prompt')
        if form.is_valid() and prompt:
            # Save the uploaded image
            uploaded_image = form.save(commit=False)
            uploaded_image.uploaded_at = timezone.now()
            uploaded_image.save()
            ...
            if not gemini_file:
                ...
                try:
                    # Start a chat with the uploaded image and the prompt
                    chat_session = model.start_chat(
                        history=[
                            {
                                "role": "user",
...
def document_and_prompt_to_text(request):
    """
    Handles document upload and prompt submission.
    """
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            uploaded_document.save()  # Save the document

            document_path = uploaded_document.document.path

            # Upload the document to Gemini
            gemini_file = upload_to_gemini(document_path, mime_type="application/pdf")  # Assuming PDF file for example
            if not gemini_file:
                extracted_text = "Error uploading the document to Gemini."
            else:
                # Create a chat session with the Gemini model
                generation_config = {
                    "temperature": 1,
                ...
                try:
                    # Start a chat with the uploaded document and prompt
                    chat_session = model.start_chat(
                        history=[
                            {
                                "role": "user",
                                "parts": [
                                    gemini_file,  # Uploaded document
                                    form.cleaned_data['prompt'],  # User's prompt
                                ],
                            }
                        ]
                    )
                    ...
                    # Send the prompt and receive the response
                    response = chat_session.send_message(form.cleaned_data['prompt'])
                    extracted_text = response.text or "No text found in the document."
                except Exception as e:
                    print(f"Error interacting with Gemini API: {e}")
                    extracted_text = "An error occurred while processing the document with Gemini."

            # Save the extracted text to the database
            uploaded_document.extracted_text = extracted_text
...
def temp_chat_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            uploaded_file = request.FILES.get("file")  # Handle a single file
            ...
            if uploaded_file:
                ...
                if file_extension in ["jpg", "jpeg", "png", "gif",'webp','heic','heif', "jfif"]:
                    ...
                    response = {"error": f"Unsupported file type: {file_extension}"}
            else:
                response = process_file_and_prompt(None, prompt, file_type="prompt")

            return render(request, "chatbot/temp_chat.html", {
                "form": form,
                "response": response,  # Response for the single file or prompt
            })
    else:
